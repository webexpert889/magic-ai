...
class AdminController extends Controller
{
    ...
    public function adsense(){
        $list = GoogleAdsense::all();
        return view('panel.admin.adsense.index', compact('list'));
    }

    public function adsenseEdit($id){
        $adsense = GoogleAdsense::whereId($id)->firstOrFail();
        return view('panel.admin.adsense.edit', compact('adsense'));
    }

    public function adsenseUpdate(Request $request, $id){
        $adsense = GoogleAdsense::whereId($id)->firstOrFail();
        ...
        $adsense->status = $request->status ?? 0;
        $adsense->code = $request->code;
        $adsense->save();

        return redirect()->route('dashboard.admin.adsense.index')->with(['message' => __('Ad updated successfully.'), 'type' => 'success']);
    }
